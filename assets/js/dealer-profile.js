...
(function ($) {
		...
		for (let i = 0; i < files.length; i++) {
			...
			if (!allowedTypes.includes(file.type)) {
				alert(
					'File "' +
						file.name +
						'" is not a valid file type. Allowed: PDF, DOC, DOCX, JPG, PNG'
				);
				continue;
			}
		...
		const $docItem = $(`
			<div class="document-item" data-doc-id="${doc.id}">
				...
				<div class="document-actions">
					...
					<button type="button" class="document-delete-btn" data-doc-id="${
						doc.id
					}">Delete</button>
				</div>
			</div>
	...
	function showUploadProgress() {
		const $progress = $(
			'<div class="upload-progress">Uploading files... <span class="spinner"></span></div>'
		);
	...
	function showMessage(message, type) {
		const $message = $(
			`<div class="profile-message ${type}">${escapeHtml(message)}</div>`
		);
